...
export const getS3ImageUrl = (path: string) => {
  ...
  return `${import.meta.env.VITE_CDN_URL}/${import.meta.env.VITE_S3_BUCKET}/${path}`;
}
...
export const dataURLtoFile = (dataUrl: string, fileName: string): File => {
  try {
    ...
    throw new Error(`Failed to convert data URL to File: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
};
