...
const useReceiptDebt = () => {
  ...
  const update = async (id: string, data: any) => {
    const response: IHttpResponse = await request.put(`/${PREFIX_PATH}/${id}`, data);
  ...
  const updateInventoryForNewPeriod = async (id: string, payload: any) => {
    ...
    if (!response.success) {
      throw new Error(response.message || "Failed to update inventory for new period");
    }
  ...
  const updateReceiptItem = async (payload: {
    ...
    try {
      const response: IHttpResponse = await request.patch(`/${PREFIX_PATH}/item/update`, payload);
      
      // Check if the API response indicates an error
      if (response && typeof response === 'object' && 'success' in response) {
        if (!response.success) {
          // Handle API error responses with specific error messages
          const errorMessage = response.message || "Failed to update receipt item";
  ...
  const cancelReceiptDebt = async (receiptId: string, note?: string) => {
    ...
    const response: IHttpResponse = await request.patch(`/${PREFIX_PATH}/${receiptId}/cancel`, cancelData);
