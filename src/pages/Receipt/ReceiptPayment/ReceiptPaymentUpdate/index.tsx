...
import ModalSelectReceiptImport, { IReceiptImportMock } from "../ReceiptPaymentCreate/components/ModalSelectReceiptImport";
...
const ReceiptPaymentUpdateScreen: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const history = useHistory();
  const [presentToast] = useIonToast();
  const { getDetail, update } = useReceiptPayment();
    ...
    try {
      ...
      if (result) {
        ...
        if (result.supplierId && result.supplierName) {
          supplierStr = `${result.supplierId}__${result.supplierName}`;
        }

        const selectedImports: IReceiptImportMock[] = result.receiptImports
          ? result.receiptImports.map((imp: any) => ({
              id: imp.id,
              code: imp.receiptNumber,
              totalAmount: Number(imp.totalAmount),
              createdAt: imp.importDate || imp.createdAt,
              supplierId: imp.supplier?.id || result.supplierId,
            }))
          : [];
        ...
        setFormData({
          ...
          subjectName: result.expenseType !== ReceiptPaymentExpenseType.SUPPLIER_PAYMENT ? result.paymentObject || "" : "",
          amount: Math.round(Number(result.amount)).toString(),
          paymentMethod: result.paymentMethod as PaymentMethod,
          note: result.notes || "",
          selectedReceiptImports: selectedImports,
          isDirectExport: !!result.isDirectExport,
        });
      } else {
        presentToast({
      ...
      console.error("Error fetching detail for update:", err);
      presentToast({
        message: "Lỗi tải thông tin phiếu chi: " + (err as Error).message,
        color: "danger",
        duration: 2000,
        position: "top",
      });
  ...
  const handleSubmit = async (type: "draft" | "active") => {
    ...
    try {
      ...
      presentToast({
        message: type === "draft" ? "Đã cập nhật nháp phiếu chi" : "Cập nhật phiếu chi thành công",
        color: "success",
        duration: 2000,
        position: "top",
      });
      ...
      <IonPage>
        <IonHeader className="ion-no-border border-b border-gray-100">
          <IonToolbar className="bg-white">
            ...
            <IonTitle className="text-center text-[17px] font-bold text-gray-900">Cập nhật Phiếu Chi</IonTitle>
          </IonToolbar>
        </IonHeader>
        <IonContent className="bg-gray-50 flex items-center justify-center">
          <div className="text-center text-gray-500 mt-10">Đang tải dữ liệu phiếu chi...</div>
        </IonContent>
      </IonPage>
    );
  }
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>Cập nhật phiếu chi</IonTitle>
        </IonToolbar>
      </IonHeader>
...
export default ReceiptPaymentUpdateScreen;
