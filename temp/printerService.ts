...
export class XprinterService {
  ...
  /**
   * Print with proxy generation (single product with specified quantity)
   */
  async printWithProxyGeneration(
    ...
    try {
      ...
      // Use the updated printHorizontalBarcodes method for single product printing
      const product = {
        productCode: job1.productCode,
        productName: job1.productName || job1.productCode
      };

      const response = await this.printHorizontalBarcodes(product, quantity);
      
      if (response.success) {
        return {
          success: true,
          message: `✅ Đã in ${quantity} nhãn thành công`
        };
      }
  ...
  /**
   * Print a single product with specified quantity in horizontal layout (2 barcodes per line)
   * For example: quantity=4 will print 2 lines, each line has 2 barcodes
   */
  async printHorizontalBarcodes(
    ...
    try {
      if (!product || !product.productCode || product.productCode.trim() === '') {
        return {
          success: false,
          message: 'Mã sản phẩm không được để trống'
        };
      ...
      // Calculate number of lines needed (2 barcodes per line)
      const linesNeeded = Math.ceil(quantity / 2);
      
      // Create print jobs for dual barcode layout
      const printJobs: Array<[PrintJob, PrintJob?]> = [];
      ...
      for (let i = 0; i < linesNeeded; i++) {
        ...
        // Add second barcode if we haven't reached the total quantity
        const job2: PrintJob | undefined = (i * 2 + 1) < quantity ? {
          productCode: product.productCode,
          productName: product.productName || product.productCode,
          quantity: 1
        } : undefined;
        
        printJobs.push([job1, job2]);
      }

      // Print all lines
      for (const [job1, job2] of printJobs) {
        const result = await this.printDualBarcodeLabel(job1, job2, 1);
        if (!result.success) {
          return result;
        }
      ...
      return {
        success: true,
        message: `✅ Đã in ${quantity} mã vạch cho sản phẩm ${product.productCode} (${linesNeeded} dòng)`
      };
  ...
  /**
   * Print multiple barcodes with flexible layout options
   */
  async printMultipleBarcodes(
    products: Array<{ productCode: string; productName?: string }>,
    options: {
      quantity?: number;
      layout?: 'single' | 'dual' | 'auto';
      groupSize?: number;
    } = {}
  ): Promise<PrinterResponse> {
    const { quantity = 1, layout = 'auto', groupSize = 2 } = options;
    ...
    try {
      ...
      for (let i = 0; i < products.length; i++) {
        if (!products[i].productCode || products[i].productCode.trim() === '') {
          return {
            success: false,
            message: `Mã vạch sản phẩm thứ ${i + 1} không được để trống`
          };
        }
      }
      ...
      if (layout === 'auto') {
        actualLayout = products.length === 1 ? 'single' : 'dual';
      }

      if (actualLayout === 'single') {
        ...
        for (const product of products) {
          ...
          const result = await this.printBarcodeLabel(printJob, quantity);
          if (!result.success) {
            return result;
          }
        }
        ...
        return {
          success: true,
          message: `✅ Đã in ${products.length} sản phẩm thành công (${quantity} bản mỗi sản phẩm)`
        };
      } else {
        // Group products for dual printing
        const pairs: Array<[PrintJob, PrintJob?]> = [];
        ...
        for (let i = 0; i < products.length; i += groupSize) {
          ...
          const job1: PrintJob = {
            ...
            productName: product1.productName || product1.productCode,
            quantity: 1
          };
          
          const job2 = product2 ? {
            productCode: product2.productCode,
            productName: product2.productName || product2.productCode,
            quantity: 1
          } : undefined;
          
          pairs.push([job1, job2]);
        }
        
        // Print all pairs
        for (const pair of pairs) {
          const result = await this.printDualBarcodeLabel(pair[0], pair[1], quantity);
          if (!result.success) {
            return result;
          }
        }
        ...
        return {
          success: true,
          message: `✅ Đã in ${products.length} sản phẩm thành công theo layout ${actualLayout} (${quantity} bản)`
        };
      }
      
    } catch (error) {
      console.error('Multiple barcodes print error:', error);
      return {
        success: false,
        message: this.getErrorMessage(error)
      };
